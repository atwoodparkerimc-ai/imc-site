...
import { ShieldCheck, CalendarCheck, Loader2 } from "lucide-react";
...
export default function ManagerBriefingSelector({ locations }: ManagerBriefingSelectorProps) {
  ...
  return (
    <div className="bg-[var(--color-brand-card)] border border-[var(--color-brand-border)] p-6 rounded-sm shadow-xl w-full font-mono text-slate-100">
      ...
      <form onSubmit={handleSaveOverride} className="space-y-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <select
              ...
              className="w-full bg-[var(--color-brand-bg)] border border-[var(--color-brand-border)] focus:border-[var(--color-brand-blue)] rounded-sm px-4 py-3 text-xs text-white font-mono uppercase tracking-wide focus:outline-none transition-colors"
            >
              {locations.map((loc) => (
        ...
        {savedStatus && (
          <div className="flex items-center gap-2 text-xs font-mono text-[var(--color-metric-meetings,#10b981)] pt-1">
            <CalendarCheck className="w-4 h-4 text-[var(--color-metric-meetings,#10b981)]" /> Overridden and broadcasting to all devices at {selectedLoc}.
          </div>
        )}
      </form>
    </div>
