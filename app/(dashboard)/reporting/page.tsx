    ...
    <div className="w-full relative flex-1 flex flex-col justify-start sm:justify-center py-4 sm:py-8 pb-12 sm:pb-16 font-mono text-slate-100 select-none">
      ...
      <div className="flex-1 p-3.5 sm:p-6 max-w-6xl mx-auto w-full relative z-10 flex flex-col justify-center">
        ...
        <motion.header 
          ...
          className="mb-4 sm:mb-6 border border-[var(--color-brand-border)] bg-[var(--color-brand-card)] p-4 sm:p-6 rounded-sm relative overflow-hidden shadow-lg"
        >
          <div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 sm:gap-4">
                <div className="p-3.5 sm:p-4 border rounded-sm bg-[var(--color-brand-bg)] border-[var(--color-brand-border)] space-y-1">
                  <p className="text-[10px] sm:text-[11px] text-slate-400 font-bold uppercase tracking-wider">REPORTER BONUS</p>
                  <p className="text-2xl sm:text-3xl font-black text-[var(--color-brand-blue,#0088ff)] tabular-nums">+5 PTS</p>
                ...
                <div className="p-3.5 sm:p-4 border rounded-sm bg-[var(--color-brand-bg)] border-[var(--color-brand-border)] space-y-1">
                  <p className="text-[10px] sm:text-[11px] text-slate-400 font-bold uppercase tracking-wider">RECIPIENT BONUS</p>
                  <p className="text-2xl sm:text-3xl font-black text-[var(--color-brand-green,#00ff9d)] tabular-nums">+10 PTS</p>
                  <p className="text-[10px] sm:text-[11px] text-slate-300 mt-1 sm:mt-2 leading-relaxed hidden sm:block font-sans">
              ...
              <div className="p-3.5 sm:p-4 border rounded-sm bg-[var(--color-brand-bg)] border-[var(--color-brand-border)] space-y-1 sm:space-y-2">
                <p className="text-[10px] sm:text-[11px] text-slate-400 font-bold uppercase tracking-wider">DAILY LIMIT</p>
          ...
          >
            ...
            <div>
              <h2 className="text-slate-200 font-bold uppercase tracking-widest text-xs mb-4 sm:mb-6 border-b pb-3 sm:pb-4 border-[var(--color-brand-border)] flex items-center justify-between">
                <span>Safe Observation Details</span>
                <span className="text-[10px] sm:text-[11px] text-[var(--color-brand-blue,#0088ff)] font-black uppercase tracking-wider truncate max-w-[150px] sm:max-w-none text-right">
                  {profile?.location ? profile.location.toUpperCase() : "SPRINGVILLE SHOP"}
                </span>
              </h2>

              {formFeedback && (
                <div className={`mb-4 sm:mb-6 p-3 text-xs font-bold uppercase tracking-wider border rounded-sm ${
                  ...
                  [{formFeedback.isError ? "TRANSACTION FAULT" : "SUCCESS"}] {formFeedback.message}
                </div>
              )}
            ...
            {hasCompletedToday ? (
              ...
              <button 
                ...
                className="w-full min-h-[48px] mt-6 sm:mt-8 py-3.5 sm:py-4 font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-xs disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer rounded-sm shadow-md border outline-none bg-[var(--color-brand-blue,#0088ff)] border-[var(--color-brand-blue,#0088ff)] text-white hover:bg-white hover:text-black shadow-[0_0_20px_rgba(0,136,255,0.3)] active:scale-[0.98] touch-manipulation focus:outline-none"
              >
        ...
        {showCelebration && (
          ...
          >
            <motion.div 
              initial={{ scale: 0.8 }} 
              animate={{ scale: 1 }} 
              className="text-center p-6 sm:p-8 border rounded-sm shadow-2xl max-w-sm w-full bg-[var(--color-brand-card)] border-[var(--color-brand-green,#00ff9d)]"
            >
              <h2 className="text-4xl sm:text-5xl font-black uppercase tracking-tighter drop-shadow-md text-[var(--color-brand-green,#00ff9d)]">
                AWARDED
              </h2>
